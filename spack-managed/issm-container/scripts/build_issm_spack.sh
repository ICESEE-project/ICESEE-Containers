...
log(){ echo "[build_issm_spack] $*"; }
die(){ echo "[build_issm_spack][ERROR] $*" >&2; exit 1; }
...
if [[ ! -d "${ISSM_PREFIX}/.git" ]]; then
    ...
    log "ISSM already present, updating..."
...
log "Installing ISSM autotools external package..."
...
log "Configuring ISSM against Spack MPICH/PETSc..."
./configure \
  --prefix="${ISSM_PREFIX}" \
